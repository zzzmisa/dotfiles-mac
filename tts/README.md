...
この環境を使うエージェントスキル: `misa-tts-narration`（`coding-agent/skills/`。原稿から音声を作って直す手順）。
...
- **読点「、」はアクセント句を区切るがポーズ（無音）は作らない**。逆に半角スペースは
  ポーズを作るがアクセント句は区切らない（隣接する文節が融合することがある）。
  名詞の列挙は読点、間を空けたいところは半角スペース、と使い分けて両方を聴き比べる
  （`/audio_query` の結果を見るとどう区切られたか分かる）
- 固有名詞（アプリ名など）は辞書登録する（下記）。登録しないと単語が分割されて
  別々の語のように読まれる。一般語に負けて分割される語は priority を上げる
- `/audio_query` の `accent_phrases` を書き換えれば、アクセント位置を1語単位で調整できる
- エージェントが原稿から音声を作って直す手順はスキル `misa-tts-narration`（`coding-agent/skills/`）
