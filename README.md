...
2. フォルダ直下のインストーラを、利用環境を指定して実行する。
   ...
   途中で失敗したフォルダがあっても残りは続行し、最後に未完了のフォルダを一覧で表示する。
   各フォルダの出力にある `❌`（失敗）と `⚠️`（要手動対応）を確認して対応する。
...
この環境を使うエージェントスキル: `misa-tts-narration`（`coding-agent/skills/`。原稿の作り方、
読み・アクセントの実測と修正、聴き比べ、整音の手順）。
